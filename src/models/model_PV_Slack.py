...
def build_model_PV_Slack(data):
    ...
    model.pSlack = Var(model.B, model.T, domain=NonNegativeReals) #slack variable for unserved demand
    ...
    #Ensure that slack is not bigger than demand
    def slack_limit_rule(m, i, t):
        return m.pSlack[i, t] <= m.pD[i, t]
    model.SlackLimit = Constraint(model.B, model.T, rule=slack_limit_rule)
    ...
    def net_injection_rule(m, i, t):
        if i == 1:
            ...
            return m.p[i,t] == m.pDIS[i, t] - m.pCHA[i, t] - m.pD[i, t] + m.P_sub[0,t] + m.pPV_used[i, t] + m.pSlack[i, t]
        return m.p[i, t] == m.pDIS[i, t] - m.pCHA[i, t] - m.pD[i, t] + m.pPV_used[i, t] + m.pSlack[i, t]
